...
FROM (
  ...
  UNION ALL SELECT 'Qual classe Tailwind aplica display flex?', 'flex', 'display-flex', 'd-flex', 'fx', 'A', 'Programação Web', 'Tailwind CSS', 'Fácil'
  UNION ALL SELECT 'Qual propriedade do React identifica itens em listas?', 'key', 'idList', 'indexOf', 'mapKey', 'A', 'Programação Web', 'React', 'Fácil'
  UNION ALL SELECT 'Qual hook gerencia estado em componentes React?', 'useState', 'useRouter', 'useParams', 'useMemo', 'A', 'Programação Web', 'React', 'Fácil'
  UNION ALL SELECT 'Qual método percorre um array retornando outro array?', 'map', 'filter', 'find', 'reduceTo', 'A', 'Programação Web', 'JavaScript', 'Fácil'
  UNION ALL SELECT 'Qual método filtra itens de um array?', 'filter', 'map', 'push', 'join', 'A', 'Programação Web', 'JavaScript', 'Fácil'
  UNION ALL SELECT 'Qual tipo representa texto em TypeScript?', 'string', 'text', 'charlist', 'varchar', 'A', 'Programação Web', 'TypeScript', 'Fácil'
  UNION ALL SELECT 'Qual recurso do TypeScript define contrato para objetos?', 'interface', 'layout', 'schema html', 'component', 'A', 'Programação Web', 'TypeScript', 'Fácil'
) AS exemplos
